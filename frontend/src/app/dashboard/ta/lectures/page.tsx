...
export default function TALecturesPageList() {
  const { data: courses = [], isLoading } = useQuery({
    queryKey: ['taLecturesPageListDir'],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto animate-fade-in">
      <div>
        <h2 className="text-xl font-bold text-text-primary">Lectures Auxiliary Manager</h2>
        <p className="text-xs text-text-secondary mt-1">Select one of your assigned courses as a TA to upload supplementary lectures, slides, and learning materials.</p>
      </div>
          ...
          {courses.map((course: any) => (
            ...
            >
              ...
              <div className="mt-6 pt-4 border-t border-beige-100">
                <Link
                  href={`/dashboard/ta/courses/${course.id}?tab=lectures`}
                ...
                >
                  Manage Supplementary Materials <ChevronRight className="w-3.5 h-3.5" />
                </Link>
              </div>
            </div>
          ))}
