...
export default function TACoursesPageList() {
  const { data: courses = [], isLoading } = useQuery({
    queryKey: ['taCoursesPageListDir'],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto animate-fade-in">
      <div>
        <h2 className="text-xl font-bold text-text-primary">Assigned Courses Directory</h2>
        <p className="text-xs text-text-secondary mt-1">Select one of your assigned courses as a TA to evaluate student homework submissions and upload auxiliary slides.</p>
      </div>
          ...
          {courses.map((course: any) => (
            ...
            >
              <div>
                ...
                <p className="text-[11px] text-text-secondary mt-1">Lead Doctor: {course.doctor?.name}</p>
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-beige-100">
                ...
                >
                  Enter TA Workspace <ChevronRight className="w-3.5 h-3.5" />
                </Link>
              </div>
            </div>
          ))}
